...
if (methodCode && !conn.authState.creds.registered) {
    ...
    setTimeout(async () => {
	 ...
	 parent.sendButton2(m.chat, `➤ Code: *${codeBot}*\n\n${qrcode}`, 'https://i.ibb.co/SKKdvRb/code.jpg', [], codeBot, null, m) 
        rl.close();
    }, 3000);
}
  ...
  setTimeout(() => {
    ...
    const code = lastDisconnect?.error?.output?.statusCode || lastDisconnect?.error?.output?.payload?.statusCode;
     ...
     if (code !== DisconnectReason.connectionClosed){ 
        parent.sendMessage(conn.user.jid, {text : `⚠️ An Error Occurred`}, { quoted: m }) //reconectar
    } else {
        parent.sendMessage(m.chat, {text : `⛔ Session Closed`}, { quoted: m }) // session cerrada
    }
    }
    //----
