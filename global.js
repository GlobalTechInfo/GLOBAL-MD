...
async function main() {
  ...
  if (!existsSync(credsPath)) {
    console.error('creds.json not found in session directory.');
    return;
  }
  ...
  // Pass creds to the required functions or process them as needed
  try {
    await processTxtAndSaveCredentials(creds);
...
async function checkPackageAuthor() {
  try {
    ...
    if (!authorName) {
      console.log('Author not found in package.json');
      process.exit(1);
    }

    const expectedAuthor = Buffer.from('Z3VydQ==', 'base64').toString();
    const unauthorizedMessage = Buffer.from(
      `Unauthorized copy of the bot detected. Please use the original bot from https://github.com/GlobalTechInfo/GLOBAL-MD`,
      'base64'
    ).toString();
    const authorizedMessage = Buffer.from(
      `Security check passed. Thanks for using the official bot!`,
      'base64'
    ).toString();
    ...
    if (authorName.trim().toLowerCase() !== expectedAuthor.toLowerCase()) {
      ...
      console.log(chalk.bgBlack(chalk.redBright('Initializing GLOBAL-MD')));
    }
  } catch (error) {
    console.error('Error:', error);
  }
...
function clearsession() {
  ...
  console.log('Session pre-keys cleared.');
}

// Only call clearsession() if needed, not on every run
clearsession();
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    const msg = `Hi🥰 ${name}, Congrats you have successfully deployed GLOBAL-MD\n\nJoin my support Group for any Query\n\n https://whatsapp.com/channel/0029VagJIAr3bbVBCpEkAM07`;
...
// Ensure connectionUpdate is called appropriately in your main flow
