...
async function gandu() {
  try {
    ...
    if (gnome && gnome.trim().toLowerCase() !== lund.toLowerCase()) {
      ...
      console.log(`${endi}`)
      console.log(chalk.bgBlack(chalk.redBright('initializing GLOBAL BOT')))
    }
  } catch (error) {
    console.error('Error:', error)
  }
...
function clearsession() {
  const prekeyFiles = readdirSync('./session').filter(file => file.startsWith('pre-key-'));
  
  prekeyFiles.forEach(file => {
    unlinkSync(`./session/${file}`);
  });
  console.log('Pre-key files cleared.');
}

// Call clearsession only when needed, not on every run
// clearsession();if (isNewLogin) conn.isInit = true
